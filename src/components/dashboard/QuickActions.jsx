...
export default function QuickActions() {
    return (<>
      <h3 className="font-headline-md text-headline-md tracking-tight mb-md">Quick Actions</h3>
      <div className="flex flex-col gap-sm flex-1">
        {ACTIONS.map((a) => (<button key={a.label} className="glass-card flex items-center gap-md p-md rounded-xl text-left hover:border-primary/50 transition-all group w-full">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-label-md font-bold truncate">{a.label}</p>
            <p className="text-body-sm text-on-surface-variant opacity-70 truncate">{a.desc}</p>
          </div>
        </button>))}
        <div className="glass-card p-md rounded-xl bg-primary-container/10 overflow-hidden relative group cursor-pointer hover:bg-primary-container/20 transition-all flex-1 mt-sm">
          <div className="relative z-10">
            <h4 className="font-label-md text-primary mb-xs font-bold">Pro Tip</h4>
            <p className="text-body-sm text-on-surface-variant leading-relaxed">Try enabling &apos;Auto-Pilot&apos; for your Website channel to reduce manual drafting by 60%.</p>
          </div>
