...
export default function ChatDetail({ chat, onBack }) {
    return (<div className="flex flex-col h-full bg-white w-full">
      {/* Header */}
      <header className="h-16 flex items-center justify-between px-gutter border-b border-outline-variant/30 bg-white/80 backdrop-blur-xl sticky top-0 z-10 shrink-0">
        <div className="flex items-center gap-md">
          ...
          <div>
            <h3 className="font-headline-md text-body-lg font-bold text-on-surface">{chat.name}</h3>
            <div className="flex items-center gap-base">
              <span className="text-body-sm text-on-surface-variant">{chat.email}</span>
              <span className="px-2 py-0.5 bg-secondary-container text-on-secondary-container rounded-full text-[10px] font-bold">{chat.channel}</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-gutter space-y-lg custom-scrollbar bg-surface-bright/30">
        <div className="flex justify-center my-md">
          <span className="px-md py-xs bg-surface-variant/50 text-[11px] font-bold text-on-surface-variant rounded-full uppercase tracking-widest">October 24, 2023</span>
        </div>
